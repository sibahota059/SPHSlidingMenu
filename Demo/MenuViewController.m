...
-(instancetype)initWithItemTitles:(NSArray *)itemsTitle andItemImages:(NSArray *)itemsImage addToViewController:(UIViewController *)sender{
    if ((self = [super init]))
    {
        ...
        {
            menuArray=[itemsTitle mutableCopy];
            menuImageArray=[itemsImage mutableCopy];
        }
    }
    return self;
}
...
-(void)show{
    if(!isOpen){
        [UIView animateWithDuration:0.5 animations:^{
            self.view.frame = CGRectMake(xAxis, yAxis, width, height);
...
-(void)hide {
    if(isOpen){
        [UIView animateWithDuration:0.5 animations:^{
            self.view.frame = CGRectMake(-width, yAxis, width, height);
            self.menutableView.frame = CGRectMake(- self.menutableView.frame.origin.x,20, width, height);
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [self.delegate SPHSimpleSideMenu:self didSelectItemAtIndex:indexPath.row];
    
    [self.delegate SPHSimpleSideMenu:self selectedItemTitle:@"Some title"];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    imageView.image = [UIImage imageNamed:[menuImageArray objectAtIndex:indexPath.row]];
    return cell;
}
